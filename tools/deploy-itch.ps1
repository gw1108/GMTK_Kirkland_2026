<#
.SYNOPSIS
  Export the game's web build and upload it to itch.io via butler.

.DESCRIPTION
  One command = export + upload:
    1. Headless-export the Godot "Web" preset to game/build/web/.
  ...
  ONE-TIME SETUP (butler cannot do these for you):
    1. Create the game page: https://itch.io/game/new
       - Title: your game title (note the URL slug; set it in tools/itch-deploy.config.json)
...
.EXAMPLE
  ./tools/deploy-itch.ps1                     # export + push as <user>/<game>:html5
...
param(
  ...
  [string]$Game,                # itch.io game slug; falls back to tools/itch-deploy.config.json
