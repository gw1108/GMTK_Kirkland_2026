...
Subcommands:
  scan                   incremental index: new/changed/deleted files, build sheets, regen TOC
                         ...
                         also regenerates the Tiled index — see `tiled`)
  repack                 rebuild ALL contact sheets from the current catalog (descriptions kept)
  tiled                  regenerate <root>/_catalog/TILED_INDEX.md from .tsx/.tmx files: tile
                         geometry, wang/terrain sets, animations, collisions, map layouts —
                         the data agents need to configure Godot TileSets so tiles tile correctly
...
TILED_EXTS = {".tmx", ".tsx"}        # Tiled map/tileset XML — indexed as text, not thumbnailed
...
def walk_tiled(root):
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [d for d in dirnames
                       if d.lower() not in EXCLUDE_DIRS and d.upper() != "ARCHIVE"]
        for name in filenames:
            if Path(name).suffix.lower() in TILED_EXTS:
                yield Path(dirpath) / name


def write_tiled_index(root, repo):
    """Index Tiled .tsx/.tmx files into a greppable markdown table. Returns (n_tsx, n_tmx).

    .tsx files carry the ground truth for tile geometry (size/margin/spacing vs the source
    image) and wang/terrain sets (which tiles form transitions) — exactly what an agent needs
    to set up a Godot TileSet with working autotiling instead of guessing from pixels."""
    import xml.etree.ElementTree as ET
    tsx_rows, tmx_rows, bad = [], [], []
    for f in sorted(walk_tiled(root)):
        path = rel(f, repo)
        try:
            ...
            bad.append(f"`{path}` ({e})")
            continue
        if f.suffix.lower() == ".tsx":
            img = r.find("image")
            wang = [f"{ws.get('name') or '?'}({len(ws.findall('wangtile'))} tiles)"
                    for ws in r.iter("wangset")]
            animated = sum(1 for t in r.findall("tile") if t.find("animation") is not None)
            collision = sum(1 for t in r.findall("tile") if t.find("objectgroup") is not None)
            tsx_rows.append(
                f"| `{path}` | {r.get('tilewidth')}×{r.get('tileheight')} | {r.get('tilecount', '?')} "
                f"| {r.get('columns', '?')} | m{r.get('margin', 0)}/s{r.get('spacing', 0)} "
                f"| `{img.get('source') if img is not None else '—'}` "
                f"| {'; '.join(wang) if wang else '—'} | {animated} | {collision} |")
        else:
            layers = [l.get("name") or "?" for l in r.iter()
                      if l.tag in ("layer", "objectgroup", "imagelayer", "group")]
            tilesets = [ts.get("source") or ts.get("name") or "?" for ts in r.findall("tileset")]
            tmx_rows.append(
                f"| `{path}` | {r.get('width')}×{r.get('height')} @ {r.get('tilewidth')}×{r.get('tileheight')}px "
                f"| {', '.join(layers) if layers else '—'} "
                f"| {', '.join(f'`{t}`' for t in tilesets) if tilesets else '—'} |")
    catalog_dir(root).mkdir(parents=True, exist_ok=True)
    out = catalog_dir(root) / "TILED_INDEX.md"
    if not tsx_rows and not tmx_rows:
        if out.exists():
            out.unlink()
        return 0, 0
    lines = [
    ...
    if tsx_rows:
        lines += [f"## Tilesets ({len(tsx_rows)} .tsx)", "",
                  "| File | Tile | Count | Cols | Margin/Spacing | Image | Wang sets | Anim | Collision |",
                  "|---|---|---|---|---|---|---|---|---|"] + tsx_rows + [""]
    if tmx_rows:
        lines += [f"## Maps ({len(tmx_rows)} .tmx)", "",
                  "| File | Size @ tile px | Layers | Tilesets |",
                  "|---|---|---|---|"] + tmx_rows + [""]
    if bad:
        lines += ["## Unparseable", ""] + [f"- {b}" for b in bad] + [""]
    out.write_text("\n".join(lines), encoding="utf-8")
    return len(tsx_rows), len(tmx_rows)
...
def write_toc(root, repo):
    ...
    lines += ["## Contents", "",
              ...
              ""]
    if (catalog_dir(root) / "TILED_INDEX.md").exists():
        lines += [f"Tiled data: see `{root_rel}/_catalog/TILED_INDEX.md` for `.tsx` tile geometry and",
...
def cmd_scan(root, repo):
    ...
    n_tsx, n_tmx = write_tiled_index(root, repo)
    write_toc(root, repo)
    if not seen:
    ...
    print(json.dumps({"new": len(new), "updated": len(changed), "deleted": len(deleted),
                      ...
                      "tiled_indexed": {"tsx": n_tsx, "tmx": n_tmx},
...
def main():
    ...
    {"scan": lambda: cmd_scan(root, a.repo), "repack": lambda: cmd_repack(root, a.repo),
     "tiled": lambda: print(json.dumps(dict(zip(("tsx", "tmx"), write_tiled_index(root, a.repo))))),
