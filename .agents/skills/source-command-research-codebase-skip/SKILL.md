---
name: "source-command-research-codebase-skip"
...
# source-command-research-codebase-skip

Use this skill when the user asks to run the migrated source command `research-codebase-skip`.
...
**Project layout for sub-agents:** this is a Godot 4 project; game source (`.gd`, `.tscn`, resources) lives under `game/`, design docs under `design/`, and tunable numbers in `data/balance.csv` (read via `BalanceData.get_value`). Tell agents NOT to scan `game/addons/`, `game/reports/`, `SourceArt/`, any `ARCHIVE` folder, or `thoughts/` outside `thoughts/shared/`, unless the question explicitly targets them.
...
## Code References
- `game/path/to/file.gd:123` — description
- `game/path/to/scene.tscn` — description
